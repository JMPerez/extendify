...
SP.Providers.Google = function() {
    ...
    function getResultType() {
      ...
      if (resultType.indexOf('followers on Google+') !== -1 &&
          $('._Ve.kno-sh').text().indexOf('Songs') !== -1) {
          ...
          case ResultTypes.album:
            SP.Search.searchAlbum(name, function(href) {
              ...
              $.ajax({
                ...
                if (data.album) {
                  $('<iframe src="https://embed.spotify.com/?uri=' + data.album.href + '" width="250" height="80" frameborder="0" allowtransparency="true"></iframe>')
                    .appendTo(getPlaceholder());
                } else {
                  callbackNotFound();
                }
